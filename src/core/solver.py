...
from src.entities import Orientation, Passenger, Position, Seat


class SeatingSolver:
    def __init__(
        self, seats: list[Seat], passengers: list[Passenger], name: str = "Puzzle"
    ):
        self.seats = seats
        ...
        for p in self.passengers:
            for s in self.seats:
                self.assignments[(p.id, s.id)] = self.model.NewBoolVar(
                    f"p{p.id}_s{s.id}"
                )
    ...
    def _add_inclusion_constraint(self, p, s, potential_neighbors):
        ...
        if potential_neighbors:
            self.model.Add(sum(potential_neighbors) >= 1).OnlyEnforceIf(
    ...
    def _has_nuisance_conflict(self, p1, p2):
        ...
        bad_smell = (p1.hates_bad_smells and p2.smells_bad) or (
            p2.hates_bad_smells and p1.smells_bad
        )
        cologne = (p1.hates_cologne and p2.smells_cologne) or (
            p2.hates_cologne and p1.smells_cologne
        )
        talk = (p1.hates_talkative and p2.talkative) or (
            p2.hates_talkative and p1.talkative
        )
        music = (p1.hates_music and p2.plays_music) or (
            p2.hates_music and p1.plays_music
        )
    ...
    def _enforce_proximity_requirement(self, p1, filter_func):
        """Generic method to force p1 to be near at least one 
        passenger matching filter_func.
        """
    ...
    def _enforce_seat_requirement(self, p, seat_filter_func, attr_name):
        ...
        valid_vars = [
            self.assignments[(p.id, s.id)] for s in self.seats if seat_filter_func(s)
        ]
    ...
    def solve(self):
        ...
        for s in self.seats:
            self.model.AddAtMostOne(
                self.assignments[(p.id, s.id)] for p in self.passengers
            )
            ...
            for p2 in self.passengers:
                ...
                for s1 in self.seats:
                    for s2 in self.seats:
                        ...
                        if dist <= 1:
                            if (
                                p1.wants_to_be_alone
                                or p2.wants_to_be_alone
                                or (p1.dislikes_children and p2.is_child)
                                or (p2.dislikes_children and p1.is_child)
                                or is_rival
                            ):
                                self.model.Add(
                                    self.assignments[(p1.id, s1.id)]
                                    + self.assignments[(p2.id, s2.id)]
                                    <= 1
                                )
        ...
        for p in self.passengers:
            if p.prefers_window:
                self._enforce_seat_requirement(
                    p, lambda s: s.position == Position.WINDOW, "Window"
                )

            if p.prefers_forward:
                self._enforce_seat_requirement(
                    p, lambda s: s.orientation == Orientation.FORWARD, "Forward"
                )
